    ...
    public List<ScanResult> sortScanResult(List<ScanResult> scanResults){
        ...
        for (int i=0;i<scanResults.size();i++){
            ...
            if (isConfigured(scanResults.get(i).SSID) != null
                    && isConfigured(scanResults.get(i).SSID).networkId == getConnectedInfo().getNetworkId()){
        ...
        for (int i=0;i<scanResults.size();i++){
            if (isConfigured(scanResults.get(i).SSID) != null){
                Log.i("ggg","====已保存过的：" + scanResults.get(i).SSID);
        ...
        Log.i("ggg","==============================================" );
        for (ScanResult scanResult:sResults){
            Log.i("ggg","顺序：" + scanResult.SSID  + "|是否保存："+(isConfigured(scanResult.SSID) != null));
        }
        Log.i("ggg","==============================================" );
        return sResults;
    }
